...
class GaussianDiffusion(nn.Module):
    ...
    def sample(self, shape:tuple, classifier_1, y1, **model_kwargs) -> torch.Tensor:
        """
        ...
        for _ in tqdm(range(self.T),dynamic_ncols=True, disable=False):
        # for _ in tqdm(range(self.T),dynamic_ncols=True, disable=(local_rank % torch.cuda.device_count() != 0)):
            tlist -= 1
            with torch.no_grad():
                x_t = self.p_sample(x_t, tlist, classifier_1, y1, **model_kwargs)
        # x_t = torch.clamp(x_t, -1, 1) # do not clamp
        # if local_rank == 0:
    ...
    def cond_fn(self, x, classifier, y=None, emb=None, scale=1.0):
        assert y is not None
        with torch.enable_grad():
            x_in = x.detach().requires_grad_(True)
            logits, feature = classifier(x_in, emb)
            log_probs = F.log_softmax(logits, dim=-1)
            selected = log_probs[range(len(logits)), y.view(-1)]
            return torch.autograd.grad(selected.sum(), x_in)[0] * scale

    
    def ddim_p_mean_variance(self, x_t:torch.Tensor, t:torch.Tensor, prevt:torch.Tensor, eta:float, priv_classifier, priv_y, emb, w1, w2, **model_kwargs) -> torch.Tensor:
        """
        calculate the parameters of p_{theta}(x_{t-1}|x_t)
        """
        if model_kwargs == None:
    ...
    def ddim_p_sample(self, x_t:torch.Tensor, t:torch.Tensor, prevt:torch.Tensor, eta:float, priv_classifier, priv_y, emb, w1, w2, **model_kwargs) -> torch.Tensor: 
        ...
        B, C = x_t.shape[:2]
        assert t.shape == (B,), f"size of t is not batch size {B}"
        mean, var = self.ddim_p_mean_variance(x_t , t.type(dtype=torch.long), prevt.type(dtype=torch.long), eta, priv_classifier, priv_y, emb, w1, w2, **model_kwargs)
        assert torch.isnan(mean).int().sum() == 0, f"nan in tensor mean when t = {t[0]}"
        assert torch.isnan(var).int().sum() == 0, f"nan in tensor var when t = {t[0]}"
        noise = torch.randn_like(x_t)
        noise[t <= 0] = 0 
    ...
    def ddim_sample(self, shape:tuple, num_steps:int, eta:float, select:str, priv_classifier, priv_y, emb, w1=1.8, w2=0.0, **model_kwargs) -> torch.Tensor:
        # local_rank = get_rank()
        # if local_rank == 0:
        ...
        for i in tqdm(range(num_steps), dynamic_ncols=True, disable=False):
        # for i in tqdm(range(num_steps),dynamic_ncols=True, disable=(local_rank % torch.cuda.device_count() != 0)):
            with torch.no_grad():
                ...
                x_t = self.ddim_p_sample(x_t, tlist, prevt, eta, priv_classifier, priv_y, emb, w1, w2, **model_kwargs)
